...
export class MockSmsAdapter implements NotificationProviderAdapter {
  ...
  async send(payload: NotificationPayload): Promise<ProviderDispatchResult> {
    ...
    const simulatedMsgId = `mock_sms_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;

    if (process.env.NODE_ENV !== 'test') {
      console.log(`[Notification MOCK SMS] To: ${masked} | Message: "${rendered.body.substring(0, 60)}..." | ID: ${simulatedMsgId}`);
    }
